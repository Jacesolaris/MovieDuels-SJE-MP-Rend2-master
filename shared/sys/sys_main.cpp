	...
	const size_t numPaths, const uint32_t searchFlags, const char* callerName)
{
	...
	if (searchFlags & SEARCH_PATH_MovieDuels)
	{
		for (size_t i = 0; i < numPaths; i++)
		{
			const char* libDir = searchPaths[i];
			...
			fn = FS_BuildOSPath(libDir, MovieDuelsGAME, filename);
			libHandle = Sys_LoadLibrary(fn);
			if (libHandle)
				return libHandle;
...
void* Sys_LoadLegacyGameDll(const char* name, VMMainProc** vmMain, SystemCallProc* systemcalls)
{
	...
	const auto dllEntry = reinterpret_cast<DllEntryProc*>(Sys_LoadFunction(libHandle, "dllEntry"));
...
void* Sys_LoadSPGameDll(const char* name, GetGameAPIProc** GetGameAPI)
{
	...
	if (!libHandle)
	{
		...
		libHandle = Sys_LoadDllFromPaths(filename, gamedir, searchPaths, numPaths,
			SEARCH_PATH_BASE | SEARCH_PATH_MOD | SEARCH_PATH_MovieDuels | SEARCH_PATH_ROOT,
			__FUNCTION__);
		if (!libHandle)
			return nullptr;
	}
