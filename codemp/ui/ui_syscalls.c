...
void UISyscall_AddCommand(const char* cmd_name)
{
	...
	Com_Printf(S_COLOR_YELLOW "WARNING: trap->ext.AddCommand() is only supported with MovieDuels mod API!\n");
}

void UISyscall_RemoveCommand(const char* cmd_name)
{
	// TODO warn developer only
	Com_Printf(S_COLOR_YELLOW "WARNING: trap->ext.RemoveCommand() is only supported with MovieDuels mod API!\n");
}
