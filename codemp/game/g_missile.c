...
void G_StasisMissile(gentity_t* ent, gentity_t* missile, vec3_t forward)
{
	...
	//
	//
// ⭐ 6. Rocket/Thermal proximity logic preserved (fixed)
//
	if (missile->s.weapon == WP_ROCKET_LAUNCHER ||
		missile->s.weapon == WP_THERMAL)
	{
		...
		{
			const int count = G_RadiusList(missile->r.currentOrigin,
				200,
				missile,
				qtrue,
				tgt_list);
		...
		if (blow == qtrue)
		{
			...
			// Schedule immediate explosion instead of disabling thinking
...
void WP_HandleBoltBlock(gentity_t* bolt, gentity_t* blocker, trace_t* trace, vec3_t fwd)
{
	...
	else
	{
		...
		else if (manual_proj_blocking || manual_run_blocking || npc_reflection)
		{
			...
			{
				block_points_used = WP_SaberBlockCost(blocker, bolt, bolt->r.currentOrigin) * 2;
			}
