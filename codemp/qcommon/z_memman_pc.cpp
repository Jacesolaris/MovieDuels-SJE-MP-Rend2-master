...
extern "C" Q_EXPORT void* MovieDuels_minizip_malloc(int size);
extern "C" Q_EXPORT void MovieDuels_minizip_free(void* to_free);

void* MovieDuels_minizip_malloc(const int size)
{
	return Z_Malloc(size, TAG_MINIZIP, qfalse, 0);
}

void MovieDuels_minizip_free(void* to_free)
{
	Z_Free(to_free);
}
