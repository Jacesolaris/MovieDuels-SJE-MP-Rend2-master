	...
	do
	{
		...
		for (const searchpath_t* search = fs_searchpaths; search; search = search->next) {
			...
			if (search->pack && search->pack->hashTable[hash]) {
				...
				// autoexec_mp.cfg and MovieDuels-SP-default.cfg can only be loaded outside of pk3 files.
				if (isUserConfig)
				{
					continue;
				}
...
/**
	...
	@param gameName Name of the default folder (i.e. always BASEGAME = "base" in MovieDuels)
*/
...
void FS_InitFilesystem(void) {
	...
	// if we can't find MovieDuels-MP-default.cfg, assume that the paths are
	// busted and error out now, rather than getting an unreadable
	// graphics screen when the font fails to load
	if (FS_ReadFile("MovieDuels-MP-default.cfg", nullptr) <= 0) {
		Com_Error(ERR_FATAL, "Couldn't load MovieDuels-MP-default.cfg");
